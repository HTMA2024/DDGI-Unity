	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float3 BaseColor = ( _Albedo * tex2D( _BaseMap, uv_BaseMap ) ).rgb;
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 uv_NormalTex = IN.ase_texcoord5.xy * _NormalTex_ST.xy + _NormalTex_ST.zw;
				float3 unpack23 = UnpackNormalScale( tex2D( _NormalTex, uv_NormalTex ), _NormalScale );
				unpack23.z = lerp( 1, unpack23.z, saturate(_NormalScale) );

				float3 Normal = unpack23;
				float Alpha = 1;
				float AlphaClipThreshold = 0.5;
				#ifdef ASE_DEPTH_WRITE_ON
					float DepthValue = IN.positionCS.z;
		...
		Pass
		{
								 ...
								 )
			{
				...
				float4 tex2DNode29 = tex2D( _MetallicGlossMap, uv_MetallicGlossMap );

				float2 uv_OcclusionMap = IN.ase_texcoord8.xy * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
				float4 tex2DNode28 = tex2D( _OcclusionMap, uv_OcclusionMap );
				float4 lerpResult36 = lerp( float4( 1,1,1,0 ) , tex2DNode28 , _AOIntensity);

				float3 BaseColor = ( _Albedo * tex2D( _BaseMap, uv_BaseMap ) ).rgb;
				float3 Normal = unpack23;
				float3 Emission = _Emission.rgb;
				float3 Specular = 0.5;
				float Metallic = ( (tex2DNode29).rgb * _Metallic ).x;
				float Smoothness = ( tex2DNode29.a * _Smoothness );
				float Occlusion = lerpResult36.r;
				float Alpha = 1;
				float AlphaClipThreshold = 0.5;
				float AlphaClipThresholdShadow = 0.5;
				float3 BakedGI = 0;
        ...
        Pass
        {
                                      ...
                                      BuiltInTriangleIntersectionAttributes attribs : SV_IntersectionAttributes)
            {
                ...
                float2 uv = v.uv;
                float4 albedoSample = _BaseMap.SampleLevel(sampler_BaseMap, uv, 0);
